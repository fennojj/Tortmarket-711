...
export default function WelcomeBonusCard(): React.ReactElement | null {
  const { user, claimWelcomeBonus, rewardConfig } = useApp();
  ...
  const onClaim = () => {
    ...
    if (res.ok) {
      ...
      Alert.alert("Welcome!", `You received ${rewardConfig.welcomeBonusPoints.toLocaleString()} bonus points. Good luck trading.`);
    }
  };
  ...
  return (
    <View style={styles.wrap} testID="welcome-bonus-card">
      ...
      <View style={{ flex: 1 }}>
        ...
        <Text style={styles.sub}>Claim {rewardConfig.welcomeBonusPoints.toLocaleString()} pts for joining the POC launch.</Text>
      </View>
